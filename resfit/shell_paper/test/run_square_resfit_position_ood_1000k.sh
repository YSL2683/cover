...
# Script to run Residual TD3 (Vanilla ResFiT Baseline) on Square Position OOD without Additional Reward Shaping (1000k Steps, 50 Demos)
# Task        : Square Position OOD (Span: 4.875 cm x 15.875 cm)
# Base Policy : 50 Demos Diffusion Policy
# Reward      : none (No reward shaping)
...
# Base policy path (pointing to trained 50-demo diffusion policy)
...
WANDB_NAME="${TASK}_position_ood_4.875x15.875_resfit_1000k_seed${SEED}"
CUSTOM_WANDB_NAME=""
...
while [[ "$#" -gt 0 ]]; do
    case $1 in
        ...
        --res_action_reg) RES_ACTION_REG="$2"; shift ;;
...
echo "=================================================="
echo "Starting Residual TD3 Training for Square Position OOD (Vanilla ResFiT Baseline, 50 Demos, 1000k Steps)"
...
export CACHE_DIR=${PROJECT_ROOT}/scratch/square_pos_ood_resfit_50demos_${CURRENT_TIME}
