...
# Script to run Residual TD3 with Decoupled Potential-Based Reward Shaping (Decoupled PBRS)
...
if [ -n "$CUSTOM_WANDB_NAME" ]; then
    ...
    WANDB_NAME="${TASK}_table_center_ood_decoupled_pbrs_beta${BETA}_scale${P_REWARD}_200demos_1000k_seed${SEED}"
fi

echo "=================================================="
echo "Starting Residual TD3 Training with DECOUPLED PBRS (200 Demos)"
...
export CACHE_DIR=${PROJECT_ROOT}/scratch/square_table_center_ood_decoupled_200demos_${CURRENT_TIME}
